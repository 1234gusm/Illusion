(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('node:stream'), require('http-errors'), require('node:http'), require('node:https'), require('node:crypto'), require('node:util')) :
  typeof define === 'function' && define.amd ? define(['node:stream', 'http-errors', 'node:http', 'node:https', 'node:crypto', 'node:util'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.BareServer = factory(global.Stream, global.createHttpError, global.http, global.https, global.node_crypto, global.node_util));
})(this, (function (Stream, createHttpError, http, https, node_crypto, node_util) { 'use strict';

  function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

  var Stream__default = /*#__PURE__*/_interopDefaultLegacy(Stream);
  var createHttpError__default = /*#__PURE__*/_interopDefaultLegacy(createHttpError);
  ...
  class Response {
    ...
    constructor(body, init = {}) {
      if (body) {
        this.body = body instanceof Stream__default["default"] ? body : Buffer.from(body);
      }
  ...
  function writeResponse(response, res) {
    ...
    if (response.body instanceof Stream__default["default"]) {
    ...
    async routeRequest(req, res) {
      ...
      try {
        if (request.method === 'OPTIONS') {
          ...
          throw new createHttpError__default["default"].NotFound();
        }
      } catch (error) {
        if (this.config.logErrors) {
          console.error(error);
        ...
        if (!(response instanceof Response)) {
          ...
          throw new createHttpError__default["default"].InternalServerError();
        }
      }
  ...
  const httpAgent = new http__default["default"].Agent();
  ...
  async function fetch(config, request, requestHeaders, url) {
    ...
    if (url.protocol === 'https:') {
      outgoing = https__default["default"].request({ ...options,
        agent: httpsAgent
      });
    } else if (url.protocol === 'http:') {
      outgoing = http__default["default"].request({ ...options,
        agent: httpAgent
      });
    } else {
  ...
  const randomBytesAsync$1 = node_util.promisify(node_crypto.randomBytes);
  ...
  const randomBytesAsync = node_util.promisify(node_crypto.randomBytes);
